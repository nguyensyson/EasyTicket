...
- **API Gateway**: **AWS API Gateway** ở môi trường dev/prod – rate-limiting chống bot, TLS termination, điều hướng request. Ở **local**, giả lập bằng container NGINX trong `docker-compose.yml` (`infra/nginx/gateway.conf`) – route theo path tới từng service qua `host.docker.internal`, kèm rate limiting cơ bản và Swagger UI tổng hợp OpenAPI docs (`http://localhost:8000`). Xem [[structure]] để biết chi tiết cấu hình.

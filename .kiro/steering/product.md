...
- API Gateway sử dụng NGINX Ingress Controller để rate-limiting và điều hướng.
...
- **Language**: Java 17+
- **Framework**: Spring Boot 3.x (Spring Web, Spring Data JPA, Spring Kafka, Spring Cache)
- **API Gateway**: NGINX Ingress Controller (Kubernetes)
- **Database**: PostgreSQL (chính), Redis (cache + inventory)
...
- Sử dụng Flyway hoặc Liquibase cho database migration
...
### Testing
- Unit test cho business logic (JUnit 5 + Mockito)
- Integration test cho Kafka consumer/producer
- Load test cho Ticket Service (Gatling/JMeter)
